...
class AsyncRecipesResource:
    ...
    async def get(self, id_or_slug: str) -> Any:
    ...
    async def update(
        ...
        *,
        ...
        res = await self._http.request("PATCH", f"/v1/recipes/{id}", json=patch)
        return res["recipe"]
    ...
    async def update(
        ...
        *,
        ...
        for k, v in (("mode", mode), ("status", status), ("monitor_id", monitor_id)):
            if v is not None:
                patch[k] = v
        res = await self._http.request("PATCH", f"/v1/guardrails/{id}", json=patch)
        return res["guardrail"]

    async def promote(self, id: str, *, to: str) -> Any:
